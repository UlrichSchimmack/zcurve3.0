...
run_SQP <- function(
  ...
  cp.res = Compute.Power.Z.General(cp.inp = sqp.est, 
        Int.Beg = Int.Beg, Int.End = Int.End, 
        val.input = val.input, yi = yi, sei = sei)
  cp.res
...
build.dens <- function(D.X, ncp, zsds = NULL, df = NULL, 
  ...
  if (CURVE.TYPE == "z") {
    ...
    if (Folded) {
      ...
      # zero out density outside the fitted window
      Dens[xmat < Int.Beg | xmat > Int.End] <- 0
 
      #for (i in 1:components) {
      #  if (i > 1) par(new=TRUE)
      #  plot(D.X,Dens[i,],type="l") }
...
Compute.Power.Z.General = function(
    cp.inp, Int.Beg, Int.End, val.input, yi, sei) {
  ...
  # Fit over full available positive z range
  dat_full <- dat[dat$z >= 0 & dat$z <= z_max, ]
  
  if (nrow(dat_full) < min_k) {
    return(rep(NA_real_, length(z_grid)))
  }
  
  fit_full <- lm(log(sei) ~ z, data = dat_full)
  
  sei_full <- exp(predict(
    fit_full,
    newdata = data.frame(z = z_grid)
  ))
  
  # Default: full-range SE function
  sei_out <- sei_full
  
  # Optional: use shrinkage extrapolation below z_min
  if (z_min > 0) {
    
    dat_sig <- dat[dat$z >= z_min & dat$z <= z_max, ]
    ...
    if (nrow(dat_sig) >= min_k) {
      ...
      sei_crit <- exp(predict(
        fit_sig,
        newdata = data.frame(z = crit)
      ))
    ...
    return(data.frame(z_grid,sei_out))
  }
  ...
  get_zcurve_es_estimates <- function(
    ...
    # Normalize full reconstructed density to probability weights
    wX <- wX / sum(wX, na.rm = TRUE)

    # ---- Empirical-Bayes posterior-mean ncp given observed z --------------
    # Replaces the old  crit + qnorm(loc.p)  inversion, which distorted the
    # shrinkage nonlinearly (Jensen on the averaged power + qnorm saturation
    # in the tail). This is E[delta | z] under the fitted population mixture:
    #   ncp_hat(z) = sum_k w_k phi(z - mu_k) mu_k / sum_k w_k phi(z - mu_k)
    # Grid form, no integration. Selection constant on z > crit cancels.
    if(!Folded) {
       Lik <- outer(z_grid, comp_ncp, function(zz, mm) dnorm(zz - mm))
    } else { 
       Lik <- outer(z_grid, comp_ncp, function(zz, mm) dnorm(zz - mm) + dnorm(zz + mm))
    }
    ...
    ext <- is.finite(val.input) & is.finite(yi) & 
       sum(val.input < Int.Beg) > 0 & sum(val.input > Int.End) > 0 

    if (any(ext)) {

      if (min(ncp) < 0) z_grid = c(Int.Beg - xz,z_grid,Int.End + xz) 
      if (min(ncp) >= 0) z_grid = c(z_grid,Int.End + xz) 
      length(z_grid) 
  
      # tail effect-size value: raw mean, UNCORRECTED by design.
      # Past Int.End the posterior is non-identified (no density to inform
      # shrinkage), so we do not pretend to shrink it here. Flag downstream.
      es.ext.pos <- mean(yi[val.input > Int.End], na.rm = TRUE)
      if (min(ncp) < 0) es.ext.neg <- mean(yi[val.input < Int.Beg], na.rm = TRUE)
      es.ext.neg = abs(es.ext.neg)
      if (is.nan(es.ext.pos)) es.ext.pos = 0
      if (is.nan(es.ext.neg)) es.ext.neg = 0
    
      
      # observed/input selected tail proportion
      w.neg.ext  <- mean(val.input < Int.Beg, na.rm = TRUE)
      w.pos.ext  <- mean(val.input > Int.End, na.rm = TRUE)
      w.grid <- 1 - w.neg.ext - w.pos.ext
  
      # average selection probability in modeled grid
      pow.grid <- sum(loc.p * wX, na.rm = TRUE)
      pow.grid <- pmin(pmax(pow.grid, 1e-6), 1 - 1e-6)
  
      # extreme tail is effectively always selected
      pow.ext <- 1
  
      # convert selected/input weights to all-results weights
      ...
      w.all.pos.ext  <- w.all.neg.ext.raw  / denom
  
      # append artificial extreme-tail point
      es.X <- abs(c(es.ext.neg,es.X, es.ext.pos))
      wX   <- c(w.all.neg.ext,w.all.grid * wX, w.all.pos.ext)

      #plot(z_grid,es.X);abline(h=0,col="red")
    ...
    # Mean over all reconstructed positive results
    es_mean_all <- sum(es.X * wX, na.rm = TRUE)
  
    # Mean over reconstructed significant results only
    sig.X <- z_grid >= crit
  
    if (sum(sig.X) > 0 && sum(wX[sig.X], na.rm = TRUE) > 0) {
      wX_sig <- wX
      wX_sig[!sig.X] <- 0
    ...
    if (tail(z_breaks, 1) < Int.End) {
      z_breaks <- c(z_breaks, Int.End)
    }

    local_es <- matrix(
      NA,
      nrow = 5,
      ncol = length(z_breaks) - 1
    )
  
    rownames(local_es) <- c("z_min", "z_max", "weight", "mean_z", "es")
    colnames(local_es) <- paste0(
      head(z_breaks, -1), "-",
      tail(z_breaks, -1)
    )
    ...
    for (j in seq_len(length(z_breaks) - 1)) {
      ...
      # Include right endpoint in final bin
      ...
      if(length(w_bin) != length(z_grid[in_bin])) stop("not equal !!!!!!!")       
...
################################
  ...
  ext.neg.inp = length(abs(val.input[val.input < Int.Beg])) /
            length(abs(val.input[abs(val.input) > Int.Beg & 
                   abs(val.input) < Int.End]))
  ext.pos.inp = length(abs(val.input[val.input > Int.End])) /
            length(abs(val.input[abs(val.input) > Int.Beg & 
                   abs(val.input) < Int.End]))
  ...
  w.all = w.inp / pow.sel
  w.sig <- w.all * pow          # reweight population by power -> significant composition
  w.sig <- w.sig / sum(w.sig)   # renormalize to sum 1  

  
  # Extend vectors with extreme component
  if(min(ncp) < 0) pow.ext     = c(1, pow, 1)
  if(min(ncp) >= 0) pow.ext     = c(pow,     1)

  if(min(ncp) < 0) pow.dir.ext   = c(1, pow.dir,     1)
  if(min(ncp) >= 0) pow.dir.ext  = c(pow.dir,     1)

  if(min(ncp) < 0) pow.sel.ext     = c(1, pow.sel,     1)
  if(min(ncp) >= 0) pow.sel.ext     = c(pow.sel,     1)

  if(min(ncp) < 0) w.inp.ext = c(ext.neg.inp, w.inp * (1 - ext.neg.inp - ext.pos.inp), ext.pos.inp)
  if(min(ncp) >= 0) w.inp.ext   = c(w.inp * (1 - ext.pos.inp), ext.pos.inp)
  ...
  lik.mat = outer(z_grid, ncp, function(x, mu) dnorm(z_grid, mu))
  dim(lik.mat)
  wd.X    = lik.mat %*% w.all  # column vector, length(z_grid)
  #plot(z_grid,wd.X)

  # Local power: posterior-weighted average of component power
  loc.p   = (lik.mat %*% (w.all * pow)) / wd.X
  
  #names(w.all) = ncp;w.all

  #plot(z_grid,loc.p,ylim=c(0,1))
  ...
  ns_idx   <- z_grid > 0 & z_grid < qnorm(1 - 0.05)        # 0 < z < 1.645, the nonsig positive band
  ...
  #print(which(idx == TRUE))
  #print(table(z_grid > lower,z_grid < upper))
  #print(z_grid[idx])
  #stop("checkpoint")
  ...
  local_power = as.vector(tapply(lp, bins, mean))
  midpoints   = (int[-length(int)] + int[-1]) / 2
  names(local_power) = paste0("lp.", midpoints)
  ...
  print("Compute Power MAIN")

  xz = .01
  z_grid <- seq(Int.Beg, Int.End, by = .01)
...
#  }
  ...
  if (!is.null(yi)) {
   ...
   #print(length(out$wd.X));print(length(out$z_grid))
...
run_OF <- function(val.input,yi=NULL,sei=NULL,ODR,Int.Beg,Int.End,ncp,zsds, cola = "springgreen4",bw.est = .2,
  ...
  #cp.inp = of.est
  ## ---- Compute power for each bootstrap result ----
  cp.res = Compute.Power.Z.General(cp.inp = of.est, 
        Int.Beg = Int.Beg, Int.End = Int.End, 
        val.input = val.input, yi = yi, sei = sei)
  cp.res
...
### eee
  ...
  cp.inp = em.est
  ## ---- Compute power for each bootstrap result ----
  cp.res = Compute.Power.Z.General(cp.inp = em.est, 
        Int.Beg = Int.Beg, Int.End = Int.End, 
        val.input = val.input, yi = yi, sei = sei)
       ...
       boot_res <- run_bootstrap(val.input = val.input, yi = yi,sei = sei,
        ...
        if(Est.Method == "ML_gamma") { 
          ...
          zcurve.res = list(
                ...
                local_es        = rbind(res.pe$local_es_pe,matrix(NA,2,length(res.pe$local_power))), 
                fit             = res.pe$fit,
...
if (length(res.main$local_power) > 3) {
   colnames(res.main$local_power) = paste0("local_power",1:ncol(res.main$local_power))
   rownames(res.main$local_power) = c("pe","lb","ub")
}

if (length(res.main$local_es) > 3) {
   colnames(res.main$local_es) = paste0("local_es",1:ncol(res.main$local_power))
