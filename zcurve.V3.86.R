...
version <- "zcurve3(3.86)"
date <- "2026.07.02"  # Version label to appear on plots
note <- "Trying local effect size estimation !!!"
...
# inputs from z-curve fit:
#   ncp    : component means (z-scale)
#   zsds   : component SDs (z-scale)  [may be all 1 if unit-variance components]
#   w.all  : mixture weights (the UNSELECTED/EDR weights - the population ones)
#   s      : the constant sei
...
zcurve_es_variable_sei <- function(
    val.input, 
    sei,
    ncp, 
    zsds, 
  ...
  stopifnot(length(z) == length(sei))
  stopifnot(length(ncp) == length(zsds))
  stopifnot(length(ncp) == length(w))

  z <- as.numeric(z)
  sei <- as.numeric(sei)

  ok <- is.finite(z) & is.finite(sei) & sei > 0
  z <- z[ok]
  sei <- sei[ok]
  ...
  # fitted z-density from z-curve mixture
  dz_mix <- function(x) {
    sapply(x, function(xx) sum(w * dnorm(xx, mean = m, sd = sd)))
  }

  # bins over target range
  breaks <- seq(lower, Int.End, by = bin_width)
  mids <- head(breaks, -1) + bin_width / 2

  # fitted probability mass per bin
  bin_mass <- sapply(seq_along(mids), function(j) {
    integrate(dz_mix, lower = breaks[j], upper = breaks[j + 1],
              rel.tol = 1e-8)$value
  })

  # normalize to target region z > lower
  bin_mass <- bin_mass / sum(bin_mass)

  # observed significant data for SE regression
  sig <- z >= crit

  se_tol <- 1e-8

  #sei = runif(length(sei),.1,.3)
  #summary(sei);sd(sei)

  sig <- z >= crit
  log_se_sig <- log(sei[sig])

  if (sum(sig) < 5 ||
    se_model == "constant" ||
    sd(log_se_sig, na.rm = TRUE) < se_tol) {

    fit <- NULL
    pred_log_se <- rep(mean(log_se_sig, na.rm = TRUE), length(mids))
    resid_sd <- 0
    ...
    fit <- lm(log_sei ~ zval, data = fit_dat)

    pred_log_se <- predict(
      fit,
      newdata = data.frame(zval = mids)
    )

    resid_sd <- summary(fit)$sigma
  }

  # reconstruct d distribution by crossing each z-bin with imputed SE draws
  d_vals <- numeric(0)
  se_vals <- numeric(0)
  weights <- numeric(0)
  ...
  for (j in seq_along(mids)) {
    ...
    if (bin_mass[j] <= 0) next

    # observed SEs if bin has observed significant studies;
    # otherwise impute from regression
    in_bin <- z >= breaks[j] & z < breaks[j + 1]

    if (sum(in_bin) > 0 && mids[j] >= crit) {
      se_j <- sei[in_bin]
    } else {
      se_j <- exp(seq(
        pred_log_se[j] - resid_sd,
        pred_log_se[j] + resid_sd,
        length.out = n_se_draws
      ))
    }

    d_j <- mids[j] * se_j
    w_j <- rep(bin_mass[j] / length(se_j), length(se_j))

    d_vals <- c(d_vals, d_j)
    se_vals <- c(se_vals, se_j)
    weights <- c(weights, w_j)
  }
  ...
  wquantile <- function(x, w, probs = .5) {
    o <- order(x)
    x <- x[o]
    w <- w[o] / sum(w)
    cw <- cumsum(w)
  ...
  if (any(tail)) {
    ...
    # rescale reconstructed weights
    weights <- weights * (1 - tail_mass)

    tail_weights <- rep(tail_mass / length(tail_d), length(tail_d))

    d_vals  <- c(d_vals,  tail_d)
    se_vals <- c(se_vals, tail_se)
    weights <- c(weights, tail_weights)
  }

  mean_d <- wmean(d_vals, weights)
  median_d <- wquantile(d_vals, weights, .5)
  sd_obs_d <- sqrt(wvar(d_vals, weights))

  # remove sampling-error variance
  mean_se2 <- wmean(se_vals^2, weights)
  tau2 <- wvar(d_vals, weights) - mean_se2
  tau <- sqrt(max(0, tau2))

  out <- c(
    z_es_mean_pos = mean_d,
    z_es_median_pos = median_d,
    z_es_tau_pos = tau,
    z_es_sd_obs_pos = sd_obs_d
  )

  out

} #EOF es with variable sei

###

zcurve_es_fixed_sei <- function(ncp, zsds, sei, w, lower = 0) {
  ...
  vk <- sd^2 * (1 + a * lambda - lambda^2)

  mean_z <- sum(Wk * mk)

  var_obs_z <- sum(Wk * (vk + mk^2)) - mean_z^2
  sd_obs_z  <- sqrt(var_obs_z)

  tau_z <- sqrt(sum(Wk * m^2) - sum(Wk * m)^2)

  trunc_cdf <- function(x) {
    Fx <- (pnorm((x - m) / sd) - pnorm(a)) / Z
    Fx <- pmax(0, pmin(1, Fx))
    sum(Wk * Fx)
  }

  upper <- max(m + 10 * sd)
  while (trunc_cdf(upper) < .5) upper <- upper * 2

  median_z <- uniroot(
    function(x) trunc_cdf(x) - .5,
    lower = lower,
    upper = upper
  )$root

  c(
    z_es_mean_pos   = mean_z * se,
    z_es_median_pos = median_z * se,
    z_es_tau_pos    = tau_z * se,
    z_es_sd_obs_pos = sd_obs_z * se
  )
}

#####

  get_es_estimates = function(val.input,yi,sei,w.all,Int.Beg,x.lim.min,Int.End,int.loc,local.power) {

    #w.all = cp.res$w.all
    breaks = seq(x.lim.min,Int.End,int.loc) 
    nb <- length(breaks) - 1
    bin.w <- numeric(nb)
    ...
    for (j in seq_len(nb)) {
      ...
      lo.p <- pnorm((lo - ncp) / zsds)

      if (is.infinite(hi)) {
        hi.p <- rep(1, length(ncp))
      } else {
        hi.p <- pnorm((hi - ncp) / zsds)
      }

      comp.mass <- hi.p - lo.p
      bin.w[j] <- sum(w.all * comp.mass)
     }

     bin.w <- bin.w / sum(bin.w)
     length(bin.w)

     bin.w <- c(bin.w * (1-mean(val.input > Int.End)),mean(val.input > Int.End))  
     length(bin.w)

     #local.power = res.run$local.power
     bins = cut(val.input,c(breaks,Inf))
     bin.w.obs = table(bins)/sum(table(bins))
     length(bin.w.obs)
     #cbind(bin.w,bin.w.obs)
     bin.power = c(local.power,1)

     #guestimae sei of ns results
     bin.sei = tapply(sei,bins,mean) 
     sei_ref <- mean(sei[val.input >= 2 & val.input <= 2.6], na.rm = TRUE) *1.2
     bin.sei[1:(2/int.loc)] = sei_ref     

     bin.yi  = tapply(yi,bins,mean)
     bin = data.frame(
      w = unname(bin.w),
      w.obs = unname(c(bin.w.obs)),
      power = unname(bin.power),
      sei = unname(bin.sei),
      yi = unname(bin.yi)
    )
    bin

    bin$ncp <- qnorm(bin$power,crit)
    bin$est_d <- bin$ncp * bin$sei
    bin$est_d[bin$power == 1] = bin$yi[bin$power == 1]
    #bin[c("w","yi","est_d")]

    es_mean_pos = sum(bin$w*bin$est_d,na.rm=TRUE)/sum(bin$w)
    es_mean_obs = sum(bin$w.obs*bin$est_d,na.rm=TRUE)/sum(bin$w.obs)
   # print(es_mean_pos)
   # print(es_mean_obs)
...
run_OF <- function(val.input,yi=NULL,sei=NULL,n_grid,ODR,Int.Beg,Int.End,ncp,zsds, cola = "springgreen4",bw.est = .2,
...
run_bootstrap <- function(
      ...
      if (!is.null(yi)) var.list = c(var.list,"yi","sei",
               "get_es_estimates")  ### "zcurve_es_fixed_sei","zcurve_es_variable_sei")
...
zcurve.time = system.time({
  ...
  #print("ODR check start run_zcurve")
  #print(ODR)
...
g_ncp = ncp
g_zsds = zsds
#ncp = g_ncp;zsds=g_zsds

if(!skip_zcurve) {

#cluster.id = NULL
### Heterogeneity Estimation
Heterogeneity.Test = FALSE
if(Heterogeneity.Test == TRUE) {
...
if (length(res.main$local.power) > 3) {
   colnames(res.main$local.power) = paste0("local.power",1:ncol(res.main$local.power))
   rownames(res.main$local.power) = c("pe","lb","ub")
}
...
results = list(
		...
		local.power    = res.main$local.power,
		BIAS           = BIAS,
		ODR_EDR_D      = ODR_EDR_D,
