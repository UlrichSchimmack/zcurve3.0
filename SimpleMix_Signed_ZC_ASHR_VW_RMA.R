...
which(settings$d[1:64] == .2)

fn_dat = "SimpleMix_signed_fixed_SE_tdist_ZC_ASHR_VW_RMA.dat"
...
for (i in b:e) {
  ...
  sel1 = sim.dat$z > -8 & sim.dat$z < 8
  sel2 = sim.dat$pop_d > -1 & sim.dat$pop_d < 1
  table(sel1,sel2)
  sel = sel1 & sel2
  table(sel)
  ...
  true_mean_all = mean(abs(sim.dat$pop_d))
  true_mean_sig = mean(abs(sim.dat$pop_d[sim.dat$z > z.crit]))
  table(cut(sim.dat$z,seq(Int.Beg,Int.End,1)))
  true_local_es = tapply(abs(sim.dat$pop_d),cut(sim.dat$z,seq(Int.Beg,Int.End,1)),mean,na.rm=TRUE)
  ...
  ash_local_es = tapply(abs(pm),cut(abs(sim.dat$z),seq(Int.Beg,Int.End,1)),mean)
  ash_mean_all = mean(abs(pm))
  ash_mean_sig = mean(abs(pm[abs(sim.dat$z) > z.crit]))
  ...
  if(length(res_run) != 131) stop("length wrong")
...
tab.comp = data.frame(tab.truth,
   z.est   = round(tab.z.pe[,4],2),
   ash.est = round(tab.ash.all.pe[,4],2),
   vw.est  = round(tab.vw.mean.pe[,4],2),
   rma.est = round(tab.rma.mean.pe[,4],2)
  )

tab.comp


summary(tab.comp[,5:8])
...
png("comparison.png", width = 8, height = 8, units = "in", res = 300)
...
plot_panel(tab.truth[,4], tab.z.pe[,4],
           "Z-curve (signed-EM)", "black")

plot_panel(tab.truth[,4], tab.ash.all.pe[,4],
           "ash-package (EM)", "darkgreen")

plot_panel(tab.truth[,4], tab.vw.mean.pe[,4],
           "weightr-package", "purple")

plot_panel(tab.truth[,4], tab.rma.mean.pe[,4],
