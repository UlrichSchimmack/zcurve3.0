...
version <- "zcurve3(3.6)"
date <- "2026.05.19"  # Version label to appear on plots
...
ODR_gr_EDR_adj = .03                # Adjustment for ODR > EDR bias test  	
  ...
  if (boot.iter > 0) {  
       ...
       boot_res <- run_bootstrap(val.input = val.input, crit=crit,
        ...
        if(Est.Method == "ML_gamma") { 
          ...
          ODR = c(res.pe$ODR,ci["odr",])
          EDR = c(res.pe$EDR,ci["EDR",])
          ...
          zcurve.res = list(
            ...
            ODR_gr_EDR_d_ci    = ODR_gr_EDR_d_ci
          )
...
bias.test = function(val.input, caliper.width = .8) {
  ...
  bias.res = run_zcurve(Est.Method="OF",val.input=val.input,crit=crit,Int.Beg = 0,Int.End = Int.End,
   ncp=ncp,zsds=zsds,cluster.id=NULL,boot.iter = 0)
...
if (length(val.input[val.input > Int.Beg & val.input < Int.End]) > 10) print("!START!") 
else {
...
hetero.test = run_zcurve(Est.Method="OF",val.input=val.input,crit=crit,
...
if (mean(hetero) < min.hetero) { 
  quick.test = run_zcurve(Est.Method="OF",val.input=val.input,crit=crit,Int.Beg=Int.Beg,Int.End=Int.End,
      ...
      NCP.FIXED=FALSE,ZSDS.FIXED=TRUE,Run.Gamma=FALSE)
...
if (hetero[1,3] < .2) { # high confidence that data are homo
...
res = run_zcurve(Est.Method = Est.Method,val.input = val.input,crit=crit,Int.Beg=Int.Beg,Int.End=Int.End,
                 ...
                 NCP.FIXED=NCP.FIXED,ZSDS.FIXED=ZSDS.FIXED,edr_adj_ci=edr.adj.ci,Run.Gamma=Run.Gamma)
...
if(!is.null(res$ODR_gr_EDR_d_ci)) BIAS$ODR_gr_EDR = res$ODR_gr_EDR_d_ci
...
if(!is.na(res$EDR[3])) FDR = FDR[c(1,3,2)]
